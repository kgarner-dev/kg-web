...
const componentDefinition = {
	...
	mapSampleUVs: (uniforms) => {
		...
		const cellCenterCorrected = floor(vec2(screenUV.x.mul(aspect), screenUV.y.oneMinus()).mul(uniforms.density.uniform)).add(.5).div(uniforms.density.uniform);
		const cellCenterScreen = vec2(cellCenterCorrected.x.div(aspect), cellCenterCorrected.y.oneMinus());
		return {
			dotSize: cellCenterScreen,
			twinkle: cellCenterScreen
		};
	},
	fragmentNode: ({ uniforms }) => {
		...
		const dot$1 = float(1).sub(smoothstep(dotSize.mul(.5).sub(pixelSize.mul(.5)), dotSize.mul(.5), centerDistance));
