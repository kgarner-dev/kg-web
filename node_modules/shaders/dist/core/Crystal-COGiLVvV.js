...
import { Fn, If, atan, clamp, convertToTexture, cos, dot, float, fract, max, min, mix, mx_worley_noise_float, pow, screenUV, sin, smoothstep, sqrt, vec2, vec3, vec4, viewportSize } from "three/tsl";
...
const componentDefinition = {
	...
	fragmentNode: ({ uniforms, childNode, onCleanup, onBeforeRender }) => {
		...
		return Fn(() => {
			...
			If(sdf.greaterThan(float(.03).add(pxH.mul(float(2)))), () => {
				const baseSample = childTexture.sample(uv$1);
				const isCutoutEarly = uniforms.cutout.uniform.equal(float(1));
				output.assign(vec4(baseSample.r, baseSample.g, baseSample.b, isCutoutEarly.select(float(0), baseSample.a)));
			}).Else(() => {
				const EPS = float(.01);
