...
function shaderRenderer() {
	...
	const updateMouseDrivers = (deltaTime) => {
		globalElapsedTime += deltaTime;
		for (const nodeInfo of nodeRegistry.nodes.values()) {
			if (!nodeInfo.mouseDriverState && !nodeInfo.autoAnimateState) continue;
			const maps = nodeInfo.metadata.maps;
			if (!maps) continue;
			for (const [propName, state] of Object.entries(nodeInfo.mouseDriverState ?? {})) {
			...
			if (!nodeInfo.autoAnimateState) continue;
			for (const [propName, state] of Object.entries(nodeInfo.autoAnimateState)) {
				const driverConfig = maps[propName];
