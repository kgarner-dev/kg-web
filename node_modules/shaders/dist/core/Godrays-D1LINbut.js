...
const componentDefinition = {
	...
	fragmentNode: (params) => {
		...
		const angle = atan(shapeUV.y, shapeUV.x);
		const angleWrapped = angle.mod(PI.mul(2));
		const blendFactor = smoothstep(-.15, .15, shapeUV.x);
		const raysShape = Fn(([a, aWrapped, blend, r, freq, intensity]) => {
			const left = vec2(a.mul(freq), r);
			const right = vec2(aWrapped.mul(freq), r);
			const nLeft = pow(valueNoise(left), intensity);
			return mix(pow(valueNoise(right), intensity), nLeft, blend);
		});
		...
		const rayEffect = Fn(() => {
			...
			const ray = raysShape(angle, angleWrapped, blendFactor, r1, f1, intensityCalc).toVar();
			ray.assign(ray.mul(raysShape(angle, angleWrapped, blendFactor, r2, f1.mul(4), intensityCalc)));
			...
			const ray2 = raysShape(angle, angleWrapped, blendFactor, r3, f2, intensityCalc).toVar();
			ray2.assign(ray2.mul(raysShape(angle, angleWrapped, blendFactor, r4, f2.mul(3.5), intensityCalc)));
