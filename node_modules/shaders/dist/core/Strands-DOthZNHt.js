...
const componentDefinition = {
	...
	fragmentNode: (params) => {
		...
		return Fn(([uv$1]) => {
			...
			const edgeFade = uniforms.pinEdges.uniform.select(smoothstep(float(0), float(.1), progress).mul(smoothstep(float(.9), float(1), progress).oneMinus()), float(1));
			...
			for (let i = 0; i < 32; i++) {
				...
				const line = smoothstep(halfWidth.sub(lineEdgeSoftness), halfWidth.add(lineEdgeSoftness), dist).oneMinus();
